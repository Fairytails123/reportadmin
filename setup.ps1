...
# Wrap everything in a script block so 'return' exits the script, not the shell.
& {
    ...
    if ($missing.Count -gt 0) {
        ...
        Write-Host "`n  Install them, then re-run this script." -ForegroundColor Red
        Pause-Exit
        return
    }
    ...
    if (Test-Path $projectDir) {
        ...
        gh repo clone Fairytails123/reportadmin $projectDir 2>&1 | Out-Host
        if ($LASTEXITCODE -ne 0) {
    ...
    try {
        ...
        Write-Host "  JotForm fields endpoint failed: $($_.Exception.Message)" -ForegroundColor Red
    }
    ...
    try {
        ...
        Write-Host "  Admin panel unreachable: $($_.Exception.Message)" -ForegroundColor Red
    }
